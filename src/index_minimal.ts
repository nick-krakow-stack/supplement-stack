...
app.get('/api/protected/products', async (c) => {
  try {
    ...
    const products = await c.env.DB.prepare(`
      SELECT p.*, 
             ...
             GROUP_CONCAT(
               json_object(
                 ...
                 'study_recommended', n.study_recommended
               )
             ) as nutrients_json
      FROM products p
      ...
      WHERE p.user_id = ? AND (p.is_duplicate = FALSE OR p.is_duplicate IS NULL)
      GROUP BY p.id
    ...
    return c.json({ success: false, error: 'Fehler beim Laden der Produkte' }, 500);
  }
});
