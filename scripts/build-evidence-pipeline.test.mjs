...
test('approved framework catalog hashes match the canonical LF file bytes', () => {
  const catalog = JSON.parse(readFileSync(resolve(ROOT, 'codex-files/frameworks/framework-catalog.v1.json'), 'utf8'))
  for (const entry of catalog.frameworks.filter(({ status }) => status === 'approved')) {
    const text = readFileSync(resolve(ROOT, entry.path), 'utf8').replaceAll('\r\n', '\n')
    assert.equal(sha256Bytes(Buffer.from(text, 'utf8')), entry.framework_sha256, `${entry.framework_id}@${entry.version}`)
  }
})
...
function setup() {
  ...
  const fit = { decision: 'existing', framework_id: 'stage2.meta_study', version: '2.0.1', variant_id: 'systematic_review_or_meta_analysis', reason: 'fit', owner_approval: { required: false, status: 'not_required' }, pilot: { required: false, status: 'not_required' } }
  const plan = {
